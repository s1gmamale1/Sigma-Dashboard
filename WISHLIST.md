...
- 🐞 **[critical] P0 — stale `no_show`/`late` record survives an OFF DAY overwrite** — importer is
...
- 🐞 **[critical] P0 — dashboard keeps going down: launchd unit never installed + port mismatch** —
...
- 🐞 **[high] P1 — no `off_day` status in the model** — closed 5-value set + DB CheckConstraint
...
- 🐞 **[high] P1 — importer silently swallows "OFF DAY"** — no branch for the literal
  (`attendance_sheet.py:124-148,57-66`); indistinguishable from a blank cell. Effort: S.
- 🐞 **[high] P1 — whole-team Sunday OFF DAY unrecognized** — "OFF DAY" in col B parses as Oliver's
...
- 🐞 **[high] P1 — frontend has no `off_day` rendering; StatusPill fails open** — unknown status →
...
- 🐞 **[high] P1 — `grade` is free text, not an enum** — `schemas.py:182`; grade bands compare
...
- 🐞 **[high] P1 — no server-side person allowlist** — `get_or_create_person` auto-creates any slug
  (`services.py:40-49`); typo spawns a phantom roster entry; allowlist only client-side
  (`dashboard_client.py:73`). Effort: S.
- 🐞 **[medium] P2 — goal `--owner` typo → ownerless goal, no error** (`services.py:140-142`). Effort: S.
...
- 🔐 **[high] P1 — Viper token compared non-constant-time** — `backend/app/auth.py:64` (`!=`), admin
  plaintext fallback `==` at `auth.py:25`. Fix: `secrets.compare_digest`. Effort: S.
- 🔐 **[high] P1 — placeholder secret defaults pass validation** — `backend/app/config.py:12,18`
...
- 🔐 **[high] P1 — no rate-limiting** on `/auth/login` or `/viper/*`. Fix: slowapi limiter. Effort: M.
...
- 🔐 **[high] P1 — raw exception strings returned to clients** — `backend/app/main.py:132`; also stored
...
- ⚙️ **[high] P1 — stray empty `backend/dashboard.db` + CWD-relative `database_url`** — launchd
  WorkingDirectory decides which DB is used. Fix: delete stray DB; pin `SIGMA_DATABASE_URL` absolute. Effort: S.

### Tests / quality

- 🧪 **[medium] P2 — zero OFF DAY tests** (`backend/tests/test_attendance_sheet.py`,
...
- 🧹 **[low] P2 — pill severity ramp inverted** — `pill-absent` teal (reads positive), `pill-no_show`
